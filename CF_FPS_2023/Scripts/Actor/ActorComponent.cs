...
public abstract class ActorComponent: MonoBehaviour,IActor
{
    ...
    public static void AddActorComponent<T>(ActorSystem tmp_actorSystem,GameObject go) where T : ActorComponent
    {
        tmp_actorSystem.AddActorComponent<T>(go.AddComponent<T>());
    }
    ...
    public virtual T GetActorComponent<T>() where T : MonoBehaviour,IActor
    {
        if (this is T)
        {
            return this as T;
