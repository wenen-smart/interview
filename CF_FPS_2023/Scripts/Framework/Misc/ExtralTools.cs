...
public static class TransformExtral
{
    ...
    public static float GetAngle(this Transform trans,Vector3 from,Vector3 to)
    {
        Vector3 cross = Vector3.Cross(from, to);
        float angle = Vector3.Angle(from, to);
    ...
    public static Vector3 GetCirclePoint(this Transform trans, float angle,float magnitude=1)
    {
        angle += trans.eulerAngles.y;
        return trans.position + new Vector3(Mathf.Sin(Mathf.Deg2Rad * angle), 0, Mathf.Cos(Mathf.Deg2Rad * angle)).normalized*magnitude;
    }
    public static Vector3 GetCirclePoint(this Transform trans, float angle, AxisType axis,float magnitude=1)
    {
        ...
        switch (axis)
        {
            case AxisType.UP:
                ...
                localOffset.x = Mathf.Sin(Mathf.Deg2Rad * angle);
                localOffset.z = Mathf.Cos(Mathf.Deg2Rad * angle);
                break;
            case AxisType.Right:
                angle += trans.localEulerAngles.x;
                localOffset.z = Mathf.Sin(Mathf.Deg2Rad * angle);
                localOffset.y = Mathf.Cos(Mathf.Deg2Rad * angle);
                break;
        ...
        return trans.position + trans.TransformVector(localOffset).normalized*magnitude;
    }
}
...
public static class VectorExtral
{
    ...
    public static bool JudgeIsLeft(this Vector3 from, Vector3 to)
    {
        Vector3 cross = Vector3.Cross(from, to);
        float angle = Vector3.Angle(from, to);
        return cross.y < 0;
