...
{
    public static bool isOpenLoaderPopup = false;
    ...
    {
        string startSceneName = "MapScene";
        #if UNITY_EDITOR
        if (EditorPrefs.GetBool(Application.productName+"_IsOpenDefaultLaunch", false)==false)
        {
            return;
        }
        startSceneName = EditorPrefs.GetString(Application.productName+"_DefaultLaunchKey", "GameLoader");
    ...
    public IEnumerator Start()
    {
        ...
        while (true)
        {
            if (GameRoot.Instance.loadScriptProgress == 1)
            {
                ...
                {
                    DebugTool.DebugLogger(MyDebugType.Warning,string.Format("加载SCENE_NAME.HOME失败，场景不存在。"));
                    yield break;
                }
                //#if UNITY_EDITOR
