...
public class GameRoot : MonoSingleTon<GameRoot>
{
    ...
    public TeamFightModeConfig TeamFightModeSO;
    public ZombieModeConfig ZombieModeSO;

    public override void Awake()
    {
        base.Awake();
        gameUI.Init();
        UIROOT = gameUI.UIROOT;
        ...
        if (RuntimeGame.Instance == null)
        {
            _Game = gameObject.AddComponent<RuntimeGame>();
            RuntimeGame.Instance.Init();
        }
    }
    ...
    private IEnumerator LoadScript(List<LoadScriptConfigData> datas)
    {
        ...
        for (int i = 0; i < datas.Count; i++)
        {
            ...
            if (scriptConfig.isContrust)
            {
                ...
                Type genericType = Type.GetType("ScriptSingleTon"+"`1");
                if (genericType!=null)
                {
