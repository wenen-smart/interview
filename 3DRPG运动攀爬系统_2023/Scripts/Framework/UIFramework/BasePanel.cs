...
public abstract class BasePanel : MonoBehaviour {
    ...
    {
        DebugTool.DebugPrint($"{Definition.GetUIPanelIdentity()}界面完成初始化");
    }
    /// <summary>
    /// 打开界面时触发 重启界面不触发。
    /// </summary>
   ...
   public virtual void Exit()
    ...
    public virtual void Hide()
    {
        ...
        {
            DebugTool.DebugSupperError($"{Definition.GetUIPanelIdentity()}-界面隐藏失败");
        }
        else
        {
            DebugTool.DebugPrint($"[UIManager:]触发{Definition.GetUIPanelIdentity()}隐藏");
        }
    }
